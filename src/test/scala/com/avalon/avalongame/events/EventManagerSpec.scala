...
class EventManagerSpec extends FunSuite with Matchers with ScalaCheckPropertyChecks with enumeratum.ScalacheckInstances {
  ...
  test("Make sure we send the TeamAssignment message to _everyone_ when mission is proposed") {
  ...
  test("Make sure we send the TeamAssignmentPhase message to _everyone_ when a proposed party is Failed") {
    forAll { (roomId: RoomId) =>
      new context {
  ...
  test("Make sure we send out AssassinOutgoingEvent when we get that back from the questResultsSeen method") {
    forAll { (roomId: RoomId) =>
      new context {
  ...
  test("Make sure we send out BadGuyVictory when we get that back from the questResultsSeen method") {
    forAll { (roomId: RoomId) =>
      new context {
  ...
  test("Make sure we send out TeamAssignmentPhase when we get that back from the questResultsSeen method") {
    forAll { (roomId: RoomId) =>
      new context {
  ...
  test("Make sure we send out GameOver when the AssassinVote is done") {
    forAll { (roomId: RoomId) =>
      new context {
